...
const index$2 = require('./index-be3a6f35.js');
...
const ScPaypalButtons = class {
  ...
  renderButtons(paypal) {
    const createFunc = index$1.hasSubscription(this.order) ? 'createBillingAgreement' : 'createOrder';
    const config = {
      /**
      ...
      onApprove: async () => {
        var _a, _b, _c, _d;
        try {
          this.order = (await index$2.fetchCheckout({ id: (_a = this.order) === null || _a === void 0 ? void 0 : _a.id }));
        }
        catch (e) {
          console.error(e);
