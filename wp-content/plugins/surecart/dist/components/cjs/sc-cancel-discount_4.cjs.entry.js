...
const ScCancelDiscount = class {
  ...
  replaceAmount(string) {
    ...
    return replaceAmount(string, price.getHumanDiscount((_b = this.protocol) === null || _b === void 0 ? void 0 : _b.preservation_coupon));
  }
  async addDiscount() {
    var _a, _b;
    try {
  ...
  render() {
    ...
    return (index.h("div", { class: "cancel-discount" }, index.h("sc-dashboard-module", { heading: this.replaceAmount(preserve_title), style: { '--sc-dashboard-module-spacing': '2em' } }, index.h("span", { slot: "description" }, this.replaceAmount(preserve_description)), index.h("sc-flex", { justifyContent: "flex-start" }, index.h("sc-button", { type: "primary", onClick: () => this.addDiscount() }, preserve_button), index.h("sc-button", { class: "cancel-discount__abort-link", type: "text", onClick: () => this.scCancel.emit() }, cancel_link)), !!this.loading && index.h("sc-block-ui", { spinner: true }))));
  }
};
