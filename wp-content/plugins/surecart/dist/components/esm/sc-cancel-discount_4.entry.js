...
import { g as getHumanDiscount } from './price-1a399968.js';
...
const ScCancelDiscount = class {
  ...
  replaceAmount(string) {
    var _a, _b;
    if (!((_a = this.protocol) === null || _a === void 0 ? void 0 : _a.preservation_coupon)) {
      return string;
    }
    return replaceAmount(string, getHumanDiscount((_b = this.protocol) === null || _b === void 0 ? void 0 : _b.preservation_coupon));
  }
  async addDiscount() {
    var _a, _b;
    try {
  ...
  render() {
    var _a;
    const { preserve_title, preserve_description, preserve_button, cancel_link } = ((_a = this.protocol) === null || _a === void 0 ? void 0 : _a.preservation_locales) || {};
    return (h("div", { class: "cancel-discount" }, h("sc-dashboard-module", { heading: this.replaceAmount(preserve_title), style: { '--sc-dashboard-module-spacing': '2em' } }, h("span", { slot: "description" }, this.replaceAmount(preserve_description)), h("sc-flex", { justifyContent: "flex-start" }, h("sc-button", { type: "primary", onClick: () => this.addDiscount() }, preserve_button), h("sc-button", { class: "cancel-discount__abort-link", type: "text", onClick: () => this.scCancel.emit() }, cancel_link)), !!this.loading && h("sc-block-ui", { spinner: true }))));
  }
};
...
const ScSubscriptionCancel = class {
  ...
  render() {
    return (h("sc-dashboard-module", { heading: this.heading || wp.i18n.__('Cancel your plan', 'surecart'), class: "subscription-cancel", error: this.error, style: { '--sc-dashboard-module-spacing': '1em' } }, this.renderContent(), h("sc-flex", { justifyContent: "flex-start" }, h("sc-button", { type: "primary", loading: this.loading || this.busy, disabled: this.loading || this.busy, onClick: () => this.cancelSubscription() }, wp.i18n.__('Cancel Plan', 'surecart')), h("sc-button", { style: { color: 'var(--sc-color-gray-500' }, type: "text", onClick: () => this.scAbandon.emit(), loading: this.loading || this.busy, disabled: this.loading || this.busy }, wp.i18n.__('Keep My Plan', 'surecart'))), this.busy && h("sc-block-ui", null)));
  }
};
